...
## 광고 PM 역량
최원혁은 인터파크 커머스에서 광고 생태계 전반을 경험했습니다:
- **디스플레이 광고**: 상품 지면 A/B 테스트, RTB 과금, 광고액 80%↑
- **검색 광고/CPS 리워드**: 바이럴 CPS 모델 0→1 기획, 4개 제휴사 온보딩
- **CRM 개인화**: Braze×GA4 연동, CPC 과금, MMP 어트리뷰션
- **DSP 운영**: 판매자 광고센터 ADMIN, RTB Second-Price + Quality Score 최적화
- **SSP 연동**: 외부 매체(넥스트페이퍼, 노티플러스) 연동 및 광고 태그 관리
- **데이터 파이프라인**: Kafka topic 기반 광고 데이터 수집 구조, ROAS 측정
- **정산**: 쿠폰 정산 로직 개선, 수수료 모델 재설계

## AD LAB (미니 광고 플랫폼)
이 포트폴리오 내에 실제 동작하는 미니 광고 생태계가 탑재되어 있습니다:
- 우측 상단 "AD ON" 토글로 광고 모드 활성화
- 쿠키 동의 → SDK 파이프라인 시각화 → 사용자 프로파일링 → RTB 비딩 → 광고 송출 → 성과 측정
- DSP/SSP/MMP/RTB 5탭 대시보드로 광고 생태계 전체를 시뮬레이션
- 방문자의 실제 브라우저 정보를 기반으로 세그먼트를 분류하고 개인화 광고를 추천
